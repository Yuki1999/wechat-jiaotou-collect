...
const Dashboard = defineComponent({
  setup() {
    ...
    async function oneClickFlow() {
      ...
      appendLog('▶ 开始完整闭环演示…\n');
      try {
        // 步骤标签：1=采集 2=AI 3=审核 4=简报 5=推送
        const labels = ['', '全量采集', 'AI 整理', '人工审核', '生成日报', '企微推送'];
        // 调用后端 /api/demo/full 一次跑完，但前端用步进动画展示
        const promise = api.post('/api/demo/full');
        // 同时做步进动画（每步 800ms）
        for (let i = 1; i <= 5; i++) {
          step.value = i;
          appendLog(`⏳ 第 ${i} 步：${labels[i]}…`);
        ...
        step.value = 0;
        showToast('演示流程已完成！6 步闭环全部成功', 'success');
      } catch (e) {
        appendLog(`❌ 出错：${e}`);
        step.value = 0;
        ...
        <div v-if="showAdd" class="card flat" style="background:var(--bg-soft);border:1px dashed var(--line-dark);margin-bottom:14px;">
          <div class="field-row">
            ...
            <div class="field"><label>类型</label>
              <select v-model="form.type">
                ...
                <option value="wechat2rss">📡 wechat2rss（公众号→RSS）</option>
          ...
          <div class="field-row mt-8">
            <div class="field" style="flex:3;">
              ...
              <div class="row" style="gap:8px;">
                ...
                <button class="btn ghost" @click="testCollect" :disabled="testing" :title="'试抓这个 URL 看看能不能解析，使用' + (form.type==='rss'||form.type==='wechat2rss' ? 'RSS' : 'crawl4ai 通用') + '引擎'">
                  {{ testing ? '⏳ 测试中…' : '🔬 测试采集' }}
                </button>
              </div>
            </div>
          ...
          <div v-if="testResult" class="card flat" style="margin-top:12px;padding:12px 14px;background:var(--bg-soft);">
            ...
            <div v-if="testResult.preview && testResult.preview.length" style="margin-top:6px;">
              <div class="hint" style="margin-bottom:4px;">列表页前 {{ testResult.preview.length }} 条：</div>
              <div v-for="(p,i) in testResult.preview" :key="i" style="padding:4px 0;font-size:12.5px;border-bottom:1px dashed var(--line);">
                <b>{{ p.title }}</b>
...
const Review = defineComponent({
  setup() {
    ...
    return {
      filter, unitFilter, kw, units, filtered,
  ...
  <div class="fade-in grid grid-2-1">
      ...
      <ul class="review-list">
        <li v-for="a in filtered" :key="a.id"
          ...
          <div class="rl-title">
            ...
            {{ a.title }}
            <span v-if="a.confidence > 0" class="ai-badge" style="margin-left:5px;" :title="'AI 置信度 '+Math.round(a.confidence*100)+'%'">AI</span>
          </div>
          <div class="rl-meta">
            <span>{{ a.unit || a.source_type }}</span>
            <span>{{ fmtTime(a.publish_time) }}</span>
    ...
    <div class="card editor">
      ...
      <div v-else>
        ...
        <div class="ai-block">
          <div class="ai-tag">🤖 AI 整理结果（可编辑）</div>
          <label>一句话摘要</label>
      ...
      <div class="card tinted" style="padding:0;">
        <div class="w2r-banner">
          ...
          <div class="w2r-banner-right">
            ...
            <button class="btn primary" @click="openSync">↻ 同步到信息源</button>
          </div>
        </div>
      </div>
